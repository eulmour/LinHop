...
static struct AudioSourceInternal {
    ma_decoder decoder;
...
AudioSource::AudioSource(const char* path, float vol) {
    ...
    this->internal = std::shared_ptr<void>(static_cast<void*>(new AudioSourceInternal()));
...
AudioSource::~AudioSource() {
    auto* internal_ptr = static_cast<AudioSourceInternal*>(this->internal.get());
    ma_decoder_uninit(&internal_ptr->decoder);
}

Audio::Audio() {

    this->internal = std::unique_ptr<AudioInternal>(new AudioInternal());
    ...
    if (ma_device_init(NULL, &internal_ptr->deviceConfig, &internal_ptr->device) != MA_SUCCESS)
...
Audio::~Audio() {
    auto* internal_ptr = static_cast<AudioInternal*>(this->internal.get());
    ma_device_uninit(&internal_ptr->device);
}
